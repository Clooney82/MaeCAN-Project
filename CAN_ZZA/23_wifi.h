...
void setup_wifi()
{
  ...
  if ( (Router_SSID == "") || (Router_Pass == "") )
  {
    ...
    { oleds[OLED_No].oled->clearBuffer();
      ...
      oleds[OLED_No].oled->drawStr(1, 10, "Please connect to WiFi:");
      oleds[OLED_No].oled->drawStr(1, 20, "SSID:");
      oleds[OLED_No].oled->drawStr(30, 20, AP_SSID.c_str());
      oleds[OLED_No].oled->drawStr(1, 30, "PW:");
      oleds[OLED_No].oled->drawStr(30, 30, AP_PASS.c_str());
...
void setup_wifi2()
{
  ESP_WiFiManager ESP_wifiManager("ConfigOnHTTP");
  ...
  */
  ...
  if ( !ESP_wifiManager.startConfigPortal(Router_SSID.c_str(), Router_Pass.c_str()) ) {
    #ifdef DEBUG_WIFI
      Serial.println("Not connected to WiFi but continuing anyway.");
    #endif
  } else {
